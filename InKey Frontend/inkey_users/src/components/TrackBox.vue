<template>
<div>
    <div class="track-box" @mouseleave="hide">
        <h2 class="track-box__index"> {{ index | twoDigits }}</h2>
        <button type="button" class="track-box__play" @click="play()">
          <font-awesome-icon :icon="isPlaying?'stop-circle':'play-circle'"/></button>
        <div class="track-box__track">
            <h3>{{name}}</h3>
            <h4>{{artist}}</h4>
        </div>
        <div class="track-box__more">
            <div class="track-box__more__duration">
                <h3>{{duration}}</h3>
            </div>
            ...
            ></star-rating>
            <slot></slot>
            <div class="track-box__more__settings">
                <button type="button" class="track-box__more__settings__btn" @click="showSettings = !showSettings" name="Open"><font-awesome-icon icon="ellipsis-h"/></button>
            </div>
            <div class="track-box__more__options-list" v-if="showSettings" v-click-outside="hide">
                <ul>
...
export default {
    ...
    methods: {
        ...
        play(){
          if(!this.isPlaying){
            EventBus.$emit("set-to-not-playing-tb");
            let song = {
              name: this.name,
              artist: this.artist,
              data: this.data,
    ...
    mounted(){
      EventBus.$on("set-to-not-playing-tb", ()=>{
        this.isPlaying = false;
      });
    },
        ...
        clickOutside: {
            bind: function (el, binding, vnode) {
                    ...
                    if (!(el == event.target || el.contains(event.target))) {
                        if(event.target.className != "track-box__more__settings__btn" && event.target.localName != "svg" && event.target.tagName != "path"){
